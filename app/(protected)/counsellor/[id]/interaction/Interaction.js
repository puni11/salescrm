...
export default function CustomerInteractions({id}) {
...
const fetchInteractions = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${!id ?'/api/dashboard/interaction?limit=100' : `/api/dashboard/interaction/${id}?limit=100`}`);
