...
import { fetchCounsellors } from "@/lib/helper/helper";
...
export default function CallLogsPage({id}) {
 ...
 useEffect(() => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await apiRequest(`${!id ? `/api/call-logs?${params.toString()}` : `/api/call-logs/${id}?${params.toString()}` }`);
